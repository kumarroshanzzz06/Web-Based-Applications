...
if($qs[1]=="a")
	$data=mysql_query("select * from stud_hostel_details order by branch,rollno");
else
{
if($qs[1]=="1")
	$qs[1]=$Y."-".$y;
	...
	$qs[1]=($Y-3)."-".($y-3);
$data=mysql_query("select * from stud_hostel_details where session='$qs[1]' order by branch,rollno");
}
